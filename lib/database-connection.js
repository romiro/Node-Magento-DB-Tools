    ...
    function runMysqlDump() {
        ...
        function downloadFile() {
            ...
            function getFile() {
                ...
                var args = [
                    util.format('%s@%s:~/%s', siteProfile['ssh_username'], siteProfile['ssh_host'], dbData.filename + ".gz"),
                    downloadFilePath
                ];
                ...
                child.stdout.on('data', function(data){
                    console.log('from scp:  ' + data);
                });

                child.stderr.on('data', function(data){
                    self.emit('error', util.format('Error with SCP while trying to download file! %s', data));
                });
            ...
            function getFile_old() {
                ...
                var command = util.format('dd if=%s bs=16k status=noxfer', dbData.filename + ".gz");
                ...
                conn.connection.exec(command, function(err, stream){
                    ...
                    intervalId = setInterval(function(){
                        console.log('Downloaded %s out of %s bytes... (last chunk size: %s)',
                            Tools.number_format(dataLength),
                            Tools.number_format(fileSize),
                            Tools.number_format(_lastChunkSize)
                        );
                    },2000);
                });
            }
