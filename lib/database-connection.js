...
DatabaseConnection.prototype.start = function(options) {
    ...
    function runMysqlDump() {
        ...
        function downloadFile() {
             ...
             * Saves as /<download directory>/<CLIENT_CODE>-<ENVIRONMENT>.sql.gz
             */
            function getFile() {
                ...
                //Ensure sanitization
                var fileName = util.format("%s-%s.sql.gz",
                    siteProfile['client_code'].replace(/[^a-zA-Z0-9]/, '').toUpperCase(),
                    siteProfile['profile_name'].replace(/[^a-zA-Z0-9]/, '').toLowerCase()
                );

                var downloadFilePath = path.join(downloadPath, fileName);

                var command = util.format('scp -o "StrictHostKeyChecking=no" %s@%s:~/%s %s',
